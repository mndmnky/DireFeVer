...
pub fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    let mut out_files = vec![
        File::create(format!("{}/lossy_contract_rule.csv",dest))?,
        File::create(format!("{}/lossy_merge_rule.csv",dest))?,
    ];

    writeln!(&mut out_files[0], "name, nk, mk, sk, uk,\
             t_lcontract, n_lcontract, m_lcontract, maxoff_lcontract,\
             t_st, n_st, m_st,\
             t_ln, n_ln, m_ln,\
             t_tn, n_tn, m_tn,\
    ...
    for (dfvsi_org, name) in instances {
        ...
        threads.push(Some(thread::spawn(move || {
            ...
            let mut dfvsi_clone = dfvsi.clone();
            let cut_stats = dfvsi_clone.apply_lossy_merge_once();
            match dfvsi_clone.exhaustive_fine_rules_stats(&priorities[0], &interrupt_receiver) {
                Ok(rule_stats) => {
                    kernels.push(dfvsi_clone.clone());
                    // Merge rules:
...
fn write_stuff(
    ...
    for i in 0..2 {
        if left_overs.len() > i {
            left_overs[i].graph.write_graph(File::create(format!("{}/{:?}_la{}k",dest,g_name,i))?)?;
            ...
            for r in 0..rule_set[i].len() {
                ...
                if r < rule_set[i].len()-1 {
                    if rule.rule == Rule::GlobalLossyContraction(2) || 
                        rule.rule == Rule::LossyMerge {
